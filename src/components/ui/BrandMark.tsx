...
export default function BrandMark({
  branding, size = 'md', showTagline = false, tone = 'ink',
}: {
  branding: Branding;
  size?: 'sm' | 'md' | 'lg';
  showTagline?: boolean;
  /** `ink` suits the portal's paper surface; `brand` is for the exam chrome. */
  tone?: 'ink' | 'brand';
}) {
  const colour = tone === 'brand' ? 'var(--brand)' : 'var(--paper-ink)';
  ...
  return (
    <span className="inline-flex items-center gap-[10px]">
      <BrandGlyph size={glyph} color={colour} />
      <span className="flex flex-col leading-none">
        <span style={{ fontSize: type, fontWeight: 600, letterSpacing: '-0.02em', color: colour }}>
          {branding.wordmark}
        </span>
        {showTagline && branding.tagline && (
          <span style={{ fontSize: Math.round(type * 0.42), marginTop: 4, color: 'var(--paper-ink-3)' }}>
            {branding.tagline}
          </span>
        )}
      </span>
