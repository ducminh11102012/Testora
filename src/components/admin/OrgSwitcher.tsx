...
export default function OrgSwitcher({
  ...
  if (options.length <= 1 && !isPlatformAdmin) {
    return (
      <div className="px-[6px]">
        <div className="text-[12px] uppercase tracking-wide text-[#8a8a8a] mb-[4px]">Organisation</div>
  ...
  return (
    <label className="block px-[6px]">
      <span className="block text-[12px] uppercase tracking-wide text-[#8a8a8a] mb-[6px]">Organisation</span>
      <select
        value={current.id}
        disabled={busy}
        onChange={async (e) => {
