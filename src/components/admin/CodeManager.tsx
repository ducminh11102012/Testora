...
export default function CodeManager({
  ...
  return (
    <div className="px-[34px] py-[34px] max-w-[1140px]">
      ...
      {batch.length > 0 && (
        <div className="border border-[color:var(--line)] rounded-[6px] p-[22px] mb-[26px]">
      ...
      {codes.length === 0 ? (
        <p className="text-[18px] text-[color:var(--paper-ink-3)]">No codes yet.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-[16px] border-collapse">
            ...
            <tbody>
              {codes.map((c) => (
                <tr key={c.id} className="border-b border-[color:var(--line)]">
                  ...
                  <td className="py-[11px] text-[color:var(--paper-ink-3)]">{c.note || '—'}</td>
                  <td className="py-[11px] text-[color:var(--paper-ink-3)]">{new Date(c.createdAt).toLocaleDateString()}</td>
