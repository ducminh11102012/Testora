...
interface Person {
  membershipId: string; id: string; displayName: string; email: string | null; username: string;
...
export default function PeopleManager({ people, orgName }: { people: Person[]; orgName: string }) {
  ...
  return (
    <div className="px-[34px] py-[34px] max-w-[1240px]">
      ...
      <p className="text-[17px] text-[color:var(--paper-ink-3)] mb-[26px]">Everyone with access to {orgName}.</p>

      <div className="border border-[color:var(--line)] rounded-[6px] mb-[28px]">
        <div className="flex border-b border-[color:var(--line)]">
          {(['one', 'bulk'] as const).map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t)}
              className={`px-[20px] py-[12px] text-[16px] ${tab === t ? 'font-semibold border-b-2 border-black' : 'text-[color:var(--paper-ink-3)]'}`}
            >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-[16px] border-collapse">
          ...
          <tbody>
            {shown.map((p) => (
              <tr key={p.membershipId} className="border-b border-[color:var(--line)]">
                <td className="py-[11px]">{p.displayName}</td>
                <td className="py-[11px] text-[color:var(--paper-ink-3)]">{p.email}</td>
