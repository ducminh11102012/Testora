...
/**
 * Parsed papers arrive as plain text as often as HTML. Rendering plain text
 * through innerHTML would run every line into one paragraph, so a block with no
 * tags in it gets its line breaks turned into real ones first.
 */
export function asHtml(text: string): string {
  if (!text) return '';
  if (/<(p|div|br|ul|ol|li|table|tr|td|th|h[1-6]|span|strong|em|b|i|u|sup|sub)\b/i.test(text)) {
    return sanitizeBlock(text);
  }
  return sanitizeBlock(text
    .split(/\n{2,}/)
