...
export interface QuestionResult {
  ...
  expected: string[];
  /** True when the marks cannot come from the answer key alone (essay tasks). */
  manual: boolean;
  /** Set when a model, rather than the answer key, decided this one. */
  markedBy?: 'ai' | 'human';
  /** The marker's or model's note to the candidate. */
  note?: string;
}
