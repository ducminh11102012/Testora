...
const RE_ANSWERKEY = /^(?:answer\s*key|answers?|key|đáp\s*án)\s*[:.]?\s*$/i;
...
function hasInlineOptions(line: string): boolean {
  return /\bA[.)]\s*\S/.test(line) && /\bB[.)]\s*\S/.test(line) && /\bC[.)]\s*\S/.test(line);
}
...
export function splitInlineOptions(text: string): { stem: string; options: ChoiceOption[] } {
  if (!hasInlineOptions(text)) return { stem: text.trim(), options: [] };
  ...
  for (const label of letters) {
    const re = new RegExp(`(?:^|[^A-Za-z])${label}[.)]\\s`, 'g');
    re.lastIndex = from;
    const m = re.exec(text);
    if (!m) break;
    const at = m.index + (m[0].length - (`${label}. `).length);
    marks.push({ label, at, len: m[0].length - (m[0].startsWith(label) ? 0 : 1) });
    from = at + 1;
  }
  ...
  const options = marks.map((mark, i) => ({
    label: mark.label,
    text: text.slice(mark.at + mark.len, i + 1 < marks.length ? marks[i + 1].at : undefined).trim(),
  }));
  return { stem, options };
}
...
export function parseWithRules(
  ...
  else if (!keyed) warnings.push('No printed answer key was found — answers will need to be filled in before the paper can be marked.');
...
function buildGroup(input: {
  ...
  for (const line of block) {
    ...
    if (option && q && q.options) { q.options.push({ label: option[1].toUpperCase(), text: option[2].trim() }); continue; }

    if (q && hasInlineOptions(line) && q.options && q.options.length === 0) {
      const { options } = splitInlineOptions(line);
      if (options.length) { q.options.push(...options); continue; }
    }
