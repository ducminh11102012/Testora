...
const COOKIE = 'examina_session';
const secret = () =>
  new TextEncoder().encode(process.env.SESSION_SECRET || 'dev-only-insecure-secret-change-me-please');
 ...
 */
export function sessionFor(userId: string, preferOrgId?: string): SessionUser | null {
  const user = users.byId(userId);
  if (!user) return null;

  const list = memberships.of(user.id);
  ...
  const org: OrganizationRow | null = chosen ? orgs.byId(chosen.orgId) : orgs.platform();
  if (!org) return null;
...
export function authenticate(login: string, password: string): SessionUser | null {
  const user = users.byLogin(login);
  if (!user) return null;
...
export function assertMember(session: SessionUser, orgId: string): boolean {
  if (session.isPlatformAdmin) return true;
  return !!memberships.find(session.id, orgId);
}
