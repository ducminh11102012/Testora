...
export const manualProvider: PaymentProvider = {
  ...
  async start({ userId, credits, amountMinor, description }) {
    const order = await orders.create({ userId, credits, amountMinor, description, provider: 'manual' });
    return { order };
  },
  async confirm(orderId, reference) {
    const order = await orders.byId(orderId);
    if (!order || order.status === 'paid') return { ok: false, credits: 0 };
    await orders.setStatus(order.id, 'paid', reference);
    await users.addCredits(order.userId, order.credits);
