...
export default async function AdminHome() {
  ...
  const papers = await tests.count(org.id);
  const published = await tests.count(org.id, 'published');
  const sittingRows = await sittings.listOrg(org.id);
  const attemptRows = await attempts.listOrg(org.id, 500);
    ...
    <div className="px-[34px] py-[34px] max-w-[1200px]">
      <div className="flex items-baseline justify-between gap-[16px] mb-[26px] flex-wrap">
        <div>
          ...
          <p className="text-[16px] text-[color:var(--paper-ink-3)] mt-[4px]">
      ...
      <div className="grid gap-[12px] sm:grid-cols-2 lg:grid-cols-5 mb-[30px]">
        ...
        <Stat label="Candidates" value={await memberships.countOrg(org.id, 'candidate')} />
          ...
          <ul className="space-y-[8px]">
            {live.slice(0, 8).map((a) => (
              <li key={a.id} className="flex items-center justify-between gap-[16px] text-[16px]">
                ...
                <span className="text-[color:var(--paper-ink-3)] tabular-nums">
      ...
      <div className="grid gap-[16px] md:grid-cols-2">
        <section className="border border-[color:var(--line)] rounded-[6px] p-[22px]">
          ...
          <p className="text-[16px] text-[color:var(--paper-ink-2)] mb-[14px]">
            {provider === 'none'
      ...
      {await imports.count(org.id) === 0 && papers === 0 && (
      ...
      <p className="mt-[26px] text-[15px] text-[color:var(--paper-ink-3)]">
