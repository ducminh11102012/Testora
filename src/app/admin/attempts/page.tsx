...
export default async function AttemptsPage() {
  ...
  const rows = attempts.listOrg(org.id, 300);

  return (
    <div className="px-[34px] py-[34px] max-w-[1240px]">
      <h1 className="text-[32px] font-semibold mb-[26px]">Attempts</h1>
      {rows.length === 0 ? (
        <p className="text-[18px] text-[#5e5e5e]">Nobody has sat a paper yet.</p>
      ) : (
            ...
            <tbody>
              {rows.map((a) => (
                <tr key={a.id} className="border-b border-[#f2f2f2]">
                  ...
                  <td className="py-[11px] text-[#5e5e5e]">{a.sessionName ?? '—'}</td>
                  ...
                  <td className="py-[11px] text-[#5e5e5e]">{new Date(a.startedAt).toLocaleString()}</td>
