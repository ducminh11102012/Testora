...
export async function POST(req: NextRequest) {
  ...
  const attempt = attempts.byId(String(attemptId ?? ''));
  if (!attempt || !sameOrg(ctx, attempt.orgId)) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  ...
  const saved = markings.forAttempt(attempt.id);
