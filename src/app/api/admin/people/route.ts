...
export async function GET() {
  ...
  return NextResponse.json({
    people: memberships.listOrg(ctx.org.id).map(({ passwordHash, ...rest }) => ({
      ...rest, attempts: users.attemptCount(rest.id),
    })),
  });
}
...
export async function POST(req: NextRequest) {
  ...
  let user = users.byEmail(String(email));
  if (!user) {
    ...
    users.update(user.id, { candidateRef: String(candidateRef) });
  }

  memberships.upsert(user.id, ctx.org.id, wanted, cohort ? String(cohort) : null);
...
export async function PUT(req: NextRequest) {
  ...
  for (const line of rows as string[]) {
    ...
    memberships.upsert(user.id, ctx.org.id, 'candidate', cohort || null);
    created.push(email);
  }
...
export async function DELETE(req: NextRequest) {
  ...
  const list = memberships.listOrg(ctx.org.id);
  if (!list.some((m) => m.membershipId === membershipId)) {
    return NextResponse.json({ error: 'Not found' }, { status: 404 });
  }
  memberships.remove(String(membershipId));
